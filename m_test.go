...
func MalformedJsonCall(t *testing.T, path string) {
	...
	resp, _ := http.Post("http://localhost:5384/"+path, "application/json", buf)
...
func TestGoodJson(t *testing.T) {
	...
	r, e := http.Post("http://localhost:5384/user/new", "application/json", bytes.NewReader(buf))
	if e != nil {
		t.Errorf("Expected success, received: %+v\n", e)
	}
	rb, e := ioutil.ReadAll(r.Body)
	fmt.Println(string(rb))
	r, e = http.Post("http://localhost:5384/user/login", "application/json", bytes.NewReader(buf))
	if e != nil {
		t.Errorf("Expected success, received: %+v\n", e)
	}
	rb, e = ioutil.ReadAll(r.Body)
	...
	if wb, e := json.Marshal(&newBell); e != nil {
		...
		if r, e := Post(token.Token, "http://localhost:5384/bell/new", wb); e != nil {
	...
	if r, e := Post(token.Token, "http://localhost:5384/bell/new", wb); e != nil || r.StatusCode != 200 {
		t.Errorf("creating bell failed %s", e)
	}

	r, e = Post(token.Token, "http://localhost:5384/bell/map", []byte{})
	reply, _ := ioutil.ReadAll(r.Body)
	fmt.Printf("map: %+v\n", string(reply))
	...
	//fmt.Printf("send: %+v\n", string(srr))
	r, e = Post(token.Token, "http://localhost:5384/send/request", srr)
	reply, _ = ioutil.ReadAll(r.Body)
	bellringerRequest := UserReply{}
	json.Unmarshal(reply, &bellringerRequest)
	fmt.Printf("send response: %+v\n", bellringerRequest)

	r, e = Post(token.Token, "http://localhost:5384/send/map", []byte{})
	reply, _ = ioutil.ReadAll(r.Body)
...
func SendTest(t *testing.T, token string, msg Message, statusCode int) {
	...
	r, e := Post(token, "http://localhost:5384/send", buf)
	reply, _  := ioutil.ReadAll(r.Body)
